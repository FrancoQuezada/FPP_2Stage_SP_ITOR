...
The first-stage decision selects a firebreak placement $y\in\mathcal{Y}$ before the fire scenario is known. In the second stage, the burned cells and the corresponding loss are determined for each scenario. The formulation below applies to a generic propagation graph $G_s=(V_s,E_s)$. The main computational experiments use the shortest-path tree $G_s^T$ defined in Section~\ref{sec2}; selected comparisons use the paired Reburn graph $G_s^R$.
...
\section{Graph-based lower-bound inequalities}\label{sec:llbi}

This section introduces valid inequalities that strengthen the scenario recourse approximations used in a Branch-and-Benders-cut framework \citep{rahmaniani2017benders}. The decomposition framework provides the solution method in which the inequalities are applied. The contribution of this paper concerns the lower-bound inequalities and their graph-based constructions.

\subsection{Branch-and-Benders framework}

The master problem contains the firebreak variables $y$ and a continuous variable $\theta_s$ that estimates the recourse loss of each scenario $s\in\mathcal{S}$. The risk aggregation remains in the master. Using the formulation from Section~\ref{s:general-model}, the master can be written as
\begin{subequations}\label{model:benders-master}
\begin{align}
\min_{y,\theta,\varphi,z}\quad
    &(1-\lambda)\sum_{s\in\mathcal{S}}\rho_s\theta_s
    +\lambda\left(
    \varphi+\frac{1}{1-\beta}
    \sum_{s\in\mathcal{S}}\rho_sz_s
    \right)
    \label{model:benders-master-objective}\\
\text{s.t.}\quad
    &y\in\mathcal{Y},
    \label{model:benders-master-budget}\\
    &z_s\geq\theta_s-\varphi,
    &&s\in\mathcal{S},
    \label{model:benders-master-excess}\\
    &z_s\geq0,
    &&s\in\mathcal{S},
    \label{model:benders-master-excess-domain}\\
    &\varphi\geq0,
    \label{model:benders-master-threshold}\\
    &\theta_s\geq Q_s(y),
    &&s\in\mathcal{S}.
    \label{model:benders-master-recourse}
\end{align}
\end{subequations}

The variables $\theta_s$ represent scenario losses before multiplication by $\rho_s$. Hence, scenario probabilities appear only in the objective. The recourse constraints~\eqref{model:benders-master-recourse} are enforced through Benders optimality cuts obtained from the linear recourse problems~\eqref{model:scenario-recourse}. In the callback implementation, these cuts are added at integer candidate solutions. Optional LP-based user cuts can also be generated at the root relaxation.

The inequalities developed below provide initial or dynamically separated lower bounds on the same variables $\theta_s$. They do not replace the Benders optimality cuts required for exactness.

\subsection{Standard lifted lower-bound inequality}

Consider a scenario $s$ without firebreaks. Let $\mathcal{B}_s^0=\mathcal{B}_s(0)$ be its burned cells and let $q_s^0=Q_s(0)=\sum_{k\in\mathcal{B}_s^0}w_k$ be its untreated loss. For each eligible non-ignition cell $i\in C\cap(\mathcal{B}_s^0\setminus\{r_s\})$, define the closed downstream set
\[
    D_s(i)=\left\{k\in\mathcal{B}_s^0:
    \text{there is a directed path from $i$ to $k$ in $G_s$}\right\}.
\]
The set includes $i$. Its weighted mass is $d_{si}=\sum_{k\in D_s(i)}w_k$. For eligible cells outside $\mathcal{B}_s^0$ and for the ignition, set $d_{si}=0$.

The standard lifted lower-bound inequality (LLBI) is
\begin{equation}
    \theta_s\geq q_s^0-\sum_{i\in C}d_{si}y_i,
    \qquad s\in\mathcal{S}.
    \label{ineq:standard-llbi}
\end{equation}

\begin{proposition}\label{prop:standard-llbi}
Inequality~\eqref{ineq:standard-llbi} is valid for every $y\in\mathcal{Y}$.
\end{proposition}

\begin{proof}
Let $\mathcal{P}_s(y)=\mathcal{B}_s^0\setminus\mathcal{B}_s(y)$ be the cells protected by $y$. Every cell in $\mathcal{P}_s(y)$ is downstream of at least one selected non-ignition cell. Therefore, $\mathcal{P}_s(y)\subseteq\bigcup_{i:y_i=1}D_s(i)$. Since the weights are nonnegative, the protected loss is at most $\sum_{i\in C}d_{si}y_i$. Subtracting this upper bound from $q_s^0$ gives~\eqref{ineq:standard-llbi}.
\end{proof}

On a propagation tree, $d_{si}$ equals the loss avoided by selecting $i$ alone. On a graph with alternative paths, it is an optimistic bound because some downstream cells may still be reached through a path that avoids $i$. The inequality remains valid, but its coefficients may count the same potential protection several times when several firebreaks are selected. CoverageLLBI and PathLLBI address this limitation using cell-level graph information.

\subsection{CoverageLLBI}

For each $k\in\mathcal{B}_s^0$, define
$\mathcal{A}_{sk}=\{i\in C\cap(\mathcal{B}_s^0\setminus\{r_s\}):
k\in D_s(i)\}$. Thus, $\mathcal{A}_{sk}$ contains the eligible cells whose
downstream sets contain $k$. Membership in this set represents potential
coverage; on a graph with alternative paths, selecting one member does not
necessarily protect $k$. Let $\mathcal{K}_s^C=\{k\in\mathcal{B}_s^0:
\mathcal{A}_{sk}\neq\emptyset\}$.

CoverageLLBI introduces a continuous variable $\zeta_{sk}$ for every
$k\in\mathcal{K}_s^C$ and adds
\begin{subequations}\label{model:coverage-llbi}
\begin{align}
    &\theta_s\geq q_s^0-
      \sum_{k\in\mathcal{K}_s^C}w_k\zeta_{sk},
      \label{model:coverage-llbi-bound}\\
    &\zeta_{sk}\leq\sum_{i\in\mathcal{A}_{sk}}y_i,
      &&k\in\mathcal{K}_s^C,
      \label{model:coverage-llbi-cover}\\
    &0\leq\zeta_{sk}\leq1,
      &&k\in\mathcal{K}_s^C.
      \label{model:coverage-llbi-domain}
\end{align}
\end{subequations}
The upper bound in~\eqref{model:coverage-llbi-domain} prevents the potential
protection of one cell from being counted more than once. For a fixed $y$,
the projected lower bound is
$q_s^0-\sum_{k\in\mathcal{K}_s^C}w_k
\min\{1,\sum_{i\in\mathcal{A}_{sk}}y_i\}$.

Eliminating $\zeta$ gives the following family in the original master
variables:
\begin{equation}
    \theta_s\geq q_s^0
    -\sum_{k\in\mathcal{K}_s^C\setminus U}w_k
    -\sum_{k\in U}w_k
       \sum_{i\in\mathcal{A}_{sk}}y_i,
    \qquad U\subseteq\mathcal{K}_s^C.
    \label{ineq:coverage-family}
\end{equation}

\begin{proposition}\label{prop:coverage-projection}
System~\eqref{model:coverage-llbi} is valid, and its projection onto
$(y,\theta_s)$ is exactly the family~\eqref{ineq:coverage-family}.
\end{proposition}

\begin{proof}
If $k$ is protected, every ignition-to-$k$ path contains a selected cell in
$\mathcal{A}_{sk}$. Hence, its protection indicator is at most
$\min\{1,\sum_{i\in\mathcal{A}_{sk}}y_i\}$, which proves validity. The
projection follows from
\[
q_s^0-\sum_{k\in\mathcal{K}_s^C}w_k
\min\left\{1,\sum_{i\in\mathcal{A}_{sk}}y_i\right\}
=\max_{U\subseteq\mathcal{K}_s^C}
\left\{q_s^0-\sum_{k\in\mathcal{K}_s^C\setminus U}w_k
-\sum_{k\in U}w_k\sum_{i\in\mathcal{A}_{sk}}y_i\right\}.
\]
\end{proof}

The complete family~\eqref{ineq:coverage-family} is exponential. At a
fractional point $\bar y$, its most violated member is obtained by taking
$U=\{k\in\mathcal{K}_s^C:
\sum_{i\in\mathcal{A}_{sk}}\bar y_i<1\}$. This separation requires only the
coverage sums and defines CoverageLLBI-exp.

CoverageLLBI-poly retains the member obtained with
$U=\mathcal{K}_s^C$:
\begin{equation}
    \theta_s\geq q_s^0-
    \sum_{k\in\mathcal{K}_s^C}w_k
    \sum_{i\in\mathcal{A}_{sk}}y_i.
    \label{ineq:coverage-poly}
\end{equation}
Reversing the order of summation shows that the coefficient of $y_i$ is
$-d_{si}$. Therefore, CoverageLLBI-poly coincides with the standard
LLBI~\eqref{ineq:standard-llbi} under the definitions used here.

\subsection{PathLLBI}

PathLLBI uses explicit propagation paths. Let $\mathcal{P}_{sk}$ be the set
of directed simple paths from $r_s$ to $k$, and let
$\widehat{\mathcal{P}}_{sk}\subseteq\mathcal{P}_{sk}$ be the paths retained
by a given construction. For a path $p$, define
$C(p)=C\cap(V(p)\setminus\{r_s\})$. The terminal cell is included when it is
eligible, whereas the ignition is always excluded. For $k=r_s$,
$\mathcal{P}_{sk}$ contains the trivial path with $C(p)=\emptyset$.

For each $k\in\mathcal{B}_s^0$, let $b_{sk}$ be a lower bound on its burn
variable. The extended PathLLBI system is
\begin{subequations}\label{model:path-llbi}
\begin{align}
    &\theta_s\geq\sum_{k\in\mathcal{B}_s^0}w_kb_{sk},
      \label{model:path-llbi-bound}\\
    &b_{sk}+\sum_{i\in C(p)}y_i\geq1,
      &&k\in\mathcal{B}_s^0,\ p\in\widehat{\mathcal{P}}_{sk},
      \label{model:path-llbi-path}\\
    &0\leq b_{sk}\leq1,
      &&k\in\mathcal{B}_s^0.
      \label{model:path-llbi-domain}
\end{align}
\end{subequations}
Constraint~\eqref{model:path-llbi-path} is valid because cell $k$ burns if
the corresponding path contains no selected firebreak. For fractional $y$,
the same inequality follows by summing the propagation constraints along the
path.

To describe the projection, let $\bot$ mean that no path is selected for a
cell. A path system $\pi$ assigns to each $k\in\mathcal{B}_s^0$ either a path
in $\widehat{\mathcal{P}}_{sk}$ or $\bot$, and
$K(\pi)=\{k:\pi_k\neq\bot\}$. The projected family is
\begin{equation}
    \theta_s\geq
    \sum_{k\in K(\pi)}w_k
    \left(1-\sum_{i\in C(\pi_k)}y_i\right),
    \qquad
    \pi\in\prod_{k\in\mathcal{B}_s^0}
    \left(\widehat{\mathcal{P}}_{sk}\cup\{\bot\}\right).
    \label{ineq:path-family}
\end{equation}

\begin{proposition}\label{prop:path-projection}
The projection of system~\eqref{model:path-llbi} onto $(y,\theta_s)$ is
exactly the family~\eqref{ineq:path-family}. If
$\widehat{\mathcal{P}}_{sk}=\mathcal{P}_{sk}$ for every $k$, this family
describes the epigraph of $Q_s$.
\end{proposition}

\begin{proof}
For fixed $y$, the smallest feasible value of $b_{sk}$ is
\[
\max\left\{0,
1-\min_{p\in\widehat{\mathcal{P}}_{sk}}
\sum_{i\in C(p)}y_i\right\}.
\]
Selecting independently one active term for each cell gives
\eqref{ineq:path-family}. When all paths are retained, let $\delta_{sk}(y)$
be the minimum path sum. Every feasible burn vector satisfies
$x_{sk}\geq[1-\delta_{sk}(y)]^+$. Conversely, the vector
$x_{sk}=[1-\delta_{sk}(y)]^+$ satisfies the propagation constraints and is
componentwise minimal. Its weighted sum is therefore $Q_s(y)$.
\end{proof}

PathLLBI-exp uses all ignition-to-cell paths without enumerating them. At a
fractional point $\bar y$, assign cost $\bar y_i$ when a path enters an
eligible non-ignition cell $i$ and zero otherwise. A single-source
shortest-path computation gives $\delta_{sk}(\bar y)$ for every cell. The
most violated member of~\eqref{ineq:path-family} selects a shortest path for
each cell with $\delta_{sk}(\bar y)<1$ and selects $\bot$ otherwise.

PathLLBI-poly retains one static member of~\eqref{ineq:path-family}. For each
cell $k$, let $\widehat p_{sk}$ be the first directed simple path found by a
depth-first traversal whose outgoing neighbors are ordered by cell index. The
resulting inequality is
\begin{equation}
    \theta_s\geq
    \sum_{k\in\mathcal{B}_s^0}w_k
    \left(1-\sum_{i\in C(\widehat p_{sk})}y_i\right).
    \label{ineq:path-poly}
\end{equation}
This rule generates one reproducible inequality per scenario. It is a valid
polynomial-size subset, not a compact representation of the complete family.

\subsection{Relations between the lower bounds}

Let $L_s^{\mathrm{LLBI}}(y)$, $L_s^C(y)$, and $L_s^P(y)$ denote the right-hand
sides of the standard LLBI, the complete coverage projection, and the complete
path projection, respectively. For $y\in[0,1]^{|C|}$,
\begin{equation}
    L_s^{\mathrm{LLBI}}(y)
    \leq L_s^C(y)
    \leq L_s^P(y)
    =Q_s(y).
    \label{eq:llbi-strength-order}
\end{equation}
The first inequality follows from the cell-level coverage caps. The second
follows because the candidates on any ignition-to-cell path form a subset of
$\mathcal{A}_{sk}$. The final equality is established in
Proposition~\ref{prop:path-projection}.

On a propagation tree, each cell has one ignition path and
$\mathcal{A}_{sk}=C(p_{sk})$. Consequently, CoverageLLBI-exp and
PathLLBI-exp define the same bound and both describe $Q_s$. The standard LLBI,
CoverageLLBI-poly, and PathLLBI-poly also coincide. On a Reburn graph,
alternative paths distinguish the constructions, and the complete path bound
dominates the complete coverage bound.

The exponential families are separated only during the prescribed root
rounds; they need not be generated completely. The Benders optimality cuts
remain responsible for exactness after root separation. Since every inequality
is stated for an individual scenario loss, it remains valid under the
expected-loss, CVaR, and mean--CVaR objectives.

\section{Computational experiments}\label{sec:computational-experiments}

This section describes the instances, compared methods, and evaluation
protocol. We distinguish optimization on the shortest-path trees, evaluation
on the paired Reburn graphs, and evaluation on scenarios not used for
optimization.

\subsection{Instances and scenario sets}

We use the \texttt{Sub20} and \texttt{Sub40} landscapes from the Alberta
region of Canada. They contain 400 and 1,600 burnable cells, respectively.
Each cell covers $100\times100$~m$^2$. The landscape data are available from
the Cell2Fire FBP repository at \url{https://github.com/fire2a/C2FFBP}.
Fire-weather scenarios combine temperature,
relative humidity, wind speed, wind direction, and fire-weather indices
required by the Canadian Forest Fire Behaviour Prediction System
\citep{hirsch1996canadian,pais2021cell2fire}. The weather data were obtained
from the Climate Information Section of Alberta Agriculture and Forestry and
the Yaha Tinda Auto station. We retain observations above the 95th percentile
of mean temperature to represent adverse fire-weather conditions.

For each landscape, the data set contains 10,000 Cell2Fire scenarios. For
every scenario $s$, the data contain the Reburn graph $G_s^R$ and the shortest-path tree
$G_s^T$ defined in Section~\ref{sec2}. Both representations have the same
ignition and reached cells, and their common scenario identifier permits a
paired comparison. All sampled scenarios are assigned probability
$\rho_s=1/|\mathcal{S}|$. We use unit cell weights, so scenario loss is the
number of burned cells.

Training scenarios are sampled without replacement from identifiers 1--9,000.
The main \texttt{Sub40} panel uses 100 and 200 training scenarios and five
independent samples for each sample size. The \texttt{Sub20} panel uses 100
training scenarios and ten independent samples. All methods use the same
training identifiers within each landscape, sample size, and replication.
Identifiers 9,001--10,000 form a fixed set of 1,000 out-of-sample scenarios
and do not enter the optimization.

\subsection{Compared methods}

Table~\ref{tab:compared-methods} separates the benchmark methods from the
proposed LLBI configurations. The extensive formulation, the baseline
Branch-and-Benders method, and Combinatorial Benders are benchmarks. The
proposed methods use the same Branch-and-Benders framework and differ only in
the lower-bound inequalities added to the master problem.

\begin{table}[htbp]
\centering
\small
\caption{Methods included in the computational comparison.}
\label{tab:compared-methods}
\begin{tabular}{p{0.16\linewidth}p{0.28\linewidth}p{0.47\linewidth}}
\toprule
Category & Method & Description \\
\midrule
Benchmark & FPP-SAA & Extensive formulation~\eqref{model:fpp}. \\
Benchmark & B\&B-RootCuts & Branch-and-Benders with LP-based root cuts and no LLBI. \\
Benchmark & Combinatorial Benders & Exact method of \citet{taninmics2024benders}, adapted to the FPP. \\
\midrule
Proposed & B\&B-LLBI & Standard LLBI~\eqref{ineq:standard-llbi}. \\
Proposed & B\&B-Coverage-exp & Separated CoverageLLBI family~\eqref{ineq:coverage-family}. \\
Proposed & B\&B-Path-poly & Static PathLLBI subset~\eqref{ineq:path-poly}. \\
Proposed & B\&B-Path-exp & Separated PathLLBI family~\eqref{ineq:path-family}. \\
\bottomrule
\end{tabular}
\end{table}

CoverageLLBI-poly is not listed separately because it coincides with the
standard LLBI. On tree scenarios, PathLLBI-poly also coincides with the
standard LLBI, while the complete CoverageLLBI and PathLLBI families define
the same bound. We retain the distinct implementations when measuring their
construction and separation costs and use Reburn scenarios to compare the
bounds when alternative propagation paths are present.

The Combinatorial Benders benchmark uses initial and fractional cuts,
heuristic lifting, and an initial 10\% scenario subset. Remaining
scenarios are checked before an incumbent is accepted, so the inspection set
changes only the search order. We test both the original order and decreasing
order of the current scenario-loss estimates. These configurations are used
only as benchmarks.

Expected loss, CVaR, and mean--CVaR are objective settings for every method.
We use $\beta=0.9$ and $\lambda=0.5$ for mean--CVaR; expected loss and pure
CVaR correspond to $\lambda=0$ and $\lambda=1$, respectively.

\subsection{Computational settings}

The treatment fractions are $\alpha\in\{0.01,0.02,0.03\}$. Each run has a
1,800-second time limit, a target relative optimality gap of $10^{-3}$, and
one solver thread. The implementation is written in C++ and uses IBM ILOG
CPLEX 22.1.1. Reported solution times include LLBI construction and root
separation.

The RootCuts configurations perform one LP-based separation round at the root
node. The exponential LLBI configurations allow at most 100 root-separation
rounds and 100 cuts per round, with a violation tolerance of $10^{-3}$. After
root separation, candidate Benders cuts preserve exactness.

% TODO: Insert the hardware and operating-system specifications used for the
% final experiment campaign.

\subsection{Evaluation protocol}

We first compare solution time, objective value, best bound, certified gap,
and the number of instances solved to the target tolerance. To assess the
effect of the proposed inequalities, we also report the root bound, LLBI
construction or separation time, and the number of root and candidate cuts.

A placement obtained from tree scenarios is evaluated in three distinct
ways. Its in-sample tree loss is computed on the training trees. Its paired
Reburn loss is then computed on the Reburn graphs with the same scenario
identifiers. This paired evaluation measures the effect of reducing the
propagation graph and is not an out-of-sample test. Finally, the placement is
evaluated on the fixed set of 1,000 unseen scenarios in the graph
representation used for optimization. We report expected burned cells,
empirical VaR at level 0.9, and empirical CVaR at level 0.9 for each
evaluation.

For the Reburn instances that can be solved within the time limit, we also
compare the tree-based placement with a placement optimized directly on the
Reburn graphs. This comparison separates the loss caused by the tree
approximation from the optimization gap. The paired scenario identifiers and
common cell set ensure that both placements are evaluated under the same fire
realizations.
