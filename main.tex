...
\subsection{Landscape and propagation graphs}

Let $V$ be the set of forest or burnable cells in the landscape. Each cell $k\in V$ has a nonnegative loss weight $w_k$. Unit weights measure the number of burned cells, whereas heterogeneous weights represent differences in the values exposed to fire.

Let $\mathcal{S}$ be a finite set of fire scenarios generated with Cell2Fire \citep{pais2021cell2fire}. Each scenario $s\in\mathcal{S}$ has probability $\rho_s$ and ignition cell $r_s$, where $\sum_{s\in\mathcal{S}}\rho_s=1$. Cell2Fire records the fire spread as a directed Reburn graph
\[
    G_s^R=(V_s,E_s^R),
\]
where $V_s\subseteq V$ contains the cells reached in the simulation and $(i,j)\in E_s^R$ indicates propagation from cell $i$ to cell $j$ \citep{pais2021downstream}. A cell may have several predecessors in $G_s^R$.

For optimization, we extract a shortest-path tree
\[
    G_s^T=(V_s,E_s^T)
\]
rooted at $r_s$. All arcs have unit length, and predecessor ties are broken by cell index. Hence, $E_s^T\subseteq E_s^R$, while both graphs contain the same cells and ignition. The tree retains every cell reachable in the Reburn graph.

Figure~\ref{fig:graphs_landscape} shows the landscape, a simulated fire, and its weather conditions. The computational experiments optimize the tree scenarios and evaluate the resulting placements on their paired Reburn graphs.

\begin{figure}[h]
\centering
\includegraphics[scale=.5]{figs/example1_sub20_and_weather.pdf}
\vspace{1ex}
\caption{A simulated fire and its weather conditions.}
\label{fig:graphs_landscape}
\end{figure}

\subsection{Firebreak placements}

Let $C\subseteq V$ be the set of cells eligible for treatment. The binary variable $y_i$ equals one if a firebreak is placed at $i\in C$. A selected non-ignition cell does not burn and blocks incoming propagation. The ignition always burns and propagates, even if it belongs to the selected set.

The parameter $\alpha\in[0,1]$ is the maximum fraction of forest or burnable cells that can be treated. The integer budget is $B=\lfloor\alpha|V|\rfloor$, and the feasible set is
\begin{equation}
    \mathcal{Y}=
...
\subsection{Scenario losses and risk measures}

For a placement $y\in\mathcal{Y}$, let $\mathcal{B}_s^g(y)$ be the cells that burn in graph $G_s^g$, where $g\in\{T,R\}$. The scenario loss is
\begin{equation}
    Q_s^g(y)=\sum_{k\in\mathcal{B}_s^g(y)}w_k.
    \label{eq:scenario-loss}
\end{equation}
Because $E_s^T\subseteq E_s^R$, $Q_s^T(y)\leq Q_s^R(y)$ for every placement. We write $Q_s(y)$ when the graph representation is fixed.
...
\end{equation}
For a fixed placement and confidence level $\beta\in(0,1)$, Conditional Value-at-Risk is \citep{rockafellar2000optimization}
\begin{equation}
...
\begin{equation}
    (1-\lambda)\mathbb{E}_{\rho}[Q(y)]
    +\lambda\operatorname{CVaR}_{\beta}(Q(y)),
    \qquad \lambda\in[0,1].
    \label{eq:mean-cvar}
\end{equation}
Thus, $\lambda=0$ gives expected loss and $\lambda=1$ gives pure CVaR.
