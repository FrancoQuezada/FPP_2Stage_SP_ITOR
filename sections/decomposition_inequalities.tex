...
\subsection{Combinatorial candidate verification}

The combinatorial variant checks a configured initial fraction of the
scenarios at each integer candidate. A detected violation rejects the
candidate; otherwise every remaining scenario is checked before it
can be accepted. The configuration also generates initial cuts and
separates fractional solutions. The initial sampling fraction thus
changes the work done before rejection, without permitting an
unchecked candidate to be accepted or changing the sampled risk
objective.
