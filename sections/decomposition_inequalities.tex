...
\subsection{Scenario ordering in combinatorial separation}
\label{sec:combinatorial-order}

The combinatorial variant orders scenarios using the current master
recourse estimates before separation. Let $\bar\eta_s$ be the value of
$\eta_s$ at the current candidate or relaxation. The policy denoted
$\eta$-asc visits scenarios in nondecreasing $\bar\eta_s$, whereas
$\eta$-desc visits them in nonincreasing $\bar\eta_s$; ties are broken
deterministically by scenario identifier. The same ordering routine is
called from the integer-candidate and fractional-separation callbacks.
Thus $\bar\eta_s$ is an optimization-state estimate of recourse, not
the realized scenario loss and not, by itself, an indicator that
scenario $s$ belongs to the CVaR tail.

With sampling fraction $r$, the integer-candidate callback initially
checks the first $\lceil r|\mathcal S|\rceil$ scenarios in this order
(at least one). A violation in that initial group rejects the candidate
without processing the remaining scenarios. If no violation is found,
every remaining scenario is checked before the candidate can be
accepted. Consequently, changing the order can change when violations
and cuts are encountered, but not the risk objective, scenario
probabilities, cut validity or the full-verification acceptance rule.
Examining large current estimates first is a plausible search heuristic
for CVaR and mean--CVaR because large recourse values can matter to
their upper-tail terms. It is not a guarantee: the estimates can be
inexact, and tail membership also depends on the evolving threshold
$t$ in \eqref{eq:master}.
