...
structural bounds, and a numerically audited comparison of six methods
on matched samples. Its 810-run reduced \texttt{new20x20} panel covers
three objectives, three training sizes and three firebreak budgets.
Certification profiles, final MIP gaps and observed times identify
different preferred configurations for expected loss and the two risk
objectives. We also establish what evidence is necessary to assess
out-of-sample stability and transfer of a fixed placement to the full
Reburn graph. The committed campaign does not contain the intact
placements, scenario graphs or scenario-level evaluations needed to
estimate these decision-quality quantities. We therefore report the
computational findings and the decision-study coverage audit without
claiming unsupported protection benefits.
...
Section~\ref{sec:computational} documents the experimental design,
computational evidence and decision-evaluation limits, followed by conclusions
