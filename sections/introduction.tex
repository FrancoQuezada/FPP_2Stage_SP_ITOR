...
structural bounds, and a comparison of six methods on matched samples.
Its 810-run reduced \texttt{new20x20} panel covers three objectives,
three training sizes and three firebreak budgets. Certification
profiles, final MIP gaps and observed times identify distinct strong
configurations by objective. A complementary decision study uses 135
Path-exp placements on fixed test scenarios to compare held-out losses
and firebreak-set similarity across training sizes. The transfer of a
fixed placement to the full Reburn graph remains a separate question.
...
solver performance and held-out decision results, followed by
