...
propagates along the directed arcs of $G_s^T$ and incurs the loss in
...
$G_s=(V_s,E_s)$; the optimization experiments use $G_s^T$.
...
$L(y)=\sum_s\rho_s Q_s^T(y)$. To represent the upper tail, let
...
\begin{equation}\label{eq:cvar}
 \operatorname{CVaR}_{\beta}(Q^T(y))
 =\min_{t\in\mathbb R}
 \left\{t+\frac{1}{1-\beta}
       \sum_{s\in\mathcal S}\rho_s[Q_s^T(y)-t]^+\right\},
 \qquad [a]^+=\max\{a,0\}.
\end{equation}
...
\begin{equation}\label{eq:risk}
 \min_{y\in\mathcal Y}
  (1-\lambda)\sum_s\rho_s Q_s^T(y)
       +\lambda\operatorname{CVaR}_{\beta}(Q^T(y)).
\end{equation}
